...
class GetIndiaDataRepository {
    ...
    companion object{
        private var globalDataRepository : GetIndiaDataRepository? = null;
        fun getInstance() : GetIndiaDataRepository{
            if(globalDataRepository == null){
                globalDataRepository = GetIndiaDataRepository();
            }
    ...
    fun getIndiaData() : MutableLiveData<CountryData>{
        var data : MutableLiveData<CountryData> = MutableLiveData<CountryData>();

        restApi.getIndiaTotalData().enqueue(object : Callback<GetGlobalDataResponse>{
