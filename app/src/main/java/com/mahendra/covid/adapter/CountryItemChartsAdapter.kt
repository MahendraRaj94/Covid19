...
class CountryItemChartsAdapter(private var items: ArrayList<CountryItemData>) :
    RecyclerView.Adapter<CountryItemChartsAdapter.ViewHolder>(),Filterable {
    private val numberFormat: DecimalFormat = DecimalFormat("##,##,##,###")

    private var filteredItems : List<CountryItemData> = items;
...
//        setBarcharts(holder, position)
    ...
    private fun setPiechartUI(pieChart: PieChart, countryData: CountryItemData) {
        ...
        pieChart.centerText = countryData.name;
    ...
    fun updateData(items: ArrayList<CountryItemData>) {
        this.items = items
        this.filteredItems = items;
        notifyDataSetChanged();
    }

    private fun generateDataPie(countryData: CountryItemData): PieData? {
        val entries = java.util.ArrayList<PieEntry>()
        entries.add(
            PieEntry(
                (countryData.confirmedCases)* 1f
    ...
    override fun getFilter(): Filter {
        return object : Filter(){
            override fun performFiltering(p0: CharSequence?): FilterResults {
                var data : List<CountryItemData> = ArrayList();
                if(p0 == null || p0.length == 0){
                    data = items;
                }else{
                    data = items.filter { item ->
                        item.name != null && item.name!!.contains(p0,true)
                    }
                }
            ...
            override fun publishResults(p0: CharSequence?, p1: FilterResults?) {
                p1?.let {
                    filteredItems = it.values as List<CountryItemData>;
                    notifyDataSetChanged()
                }
